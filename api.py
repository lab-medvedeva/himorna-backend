...
@app.route("/api/v1/peaks.zst", methods=["GET"])
def get_database_dump():
    # returns database dump file
    try:
        return send_file("/home/mazurovev/peaks.zst", attachment_filename="peaks.zst")
...
def _corr(tx, query):
    ...
    "tissue": record["tissue"] } for record in result]
